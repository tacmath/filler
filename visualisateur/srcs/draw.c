...
/*   Updated: 2019/01/30 16:08:22 by mtaquet     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static void	ft_put_player(t_map *map)
{
	...
	mlx_string_put(map->mlx_ptr, map->win_ptr, x, 220, map->p1_color, map->player1);
	x = map->size.x / 2 + 150 - (ft_strlen(map->player2) * 10) / 2;
	mlx_string_put(map->mlx_ptr, map->win_ptr, x, 220, map->p2_color, map->player2);
	tmp = ft_itoa(map->p1_point);
	x = 80 - (ft_strlen(tmp) * 10) / 2;
	mlx_string_put(map->mlx_ptr, map->win_ptr, x, map->start.y - 25, map->p1_color, tmp);
	free(tmp);
	tmp = ft_itoa(map->p2_point);
	x = map->size.x - 80 - (ft_strlen(tmp) * 10) / 2;
	mlx_string_put(map->mlx_ptr, map->win_ptr, x, map->start.y - 25, map->p2_color, tmp);
	free(tmp);
}
...
static void	ft_p1_score(t_map *map)
{
	...
	while (++y < map->pix_len * map->plateau.y + map->start.y)
	{
		...
		while (++x < 110)
		{
			if (map->pix_len * map->plateau.y - (y - map->start.y) < point_per_line * map->p1_point)
				map->data[x + y * map->size.x] = color;
			else
				map->data[x + y * map->size.x] = ft_rgb(ft_hextor(color) / 2, ft_hextog(color) / 2, ft_hextob(color) / 2);
		}
	}
}
...
static void	ft_p2_score(t_map *map)
{
	...
	while (++y < map->pix_len * map->plateau.y + map->start.y)
	{
		...
		{
			if (map->pix_len * map->plateau.y - (y - map->start.y) < point_per_line * map->p2_point)
				map->data[x + y * map->size.x] = color;
			else
				map->data[x + y * map->size.x] = ft_rgb(ft_hextor(color) / 2, ft_hextog(color) / 2, ft_hextob(color) / 2);
		}
